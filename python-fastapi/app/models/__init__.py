...
class Tool(Base):
    ...
    owner_department = Column(SQLEnum(DepartmentType, values_callable=lambda x: [e.value for e in x]), nullable=False)
    status = Column(SQLEnum(ToolStatusType, values_callable=lambda x: [e.value for e in x]), default=ToolStatusType.ACTIVE)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class User(Base):
    ...
    department = Column(SQLEnum(DepartmentType, values_callable=lambda x: [e.value for e in x]), nullable=False)
    role = Column(SQLEnum(UserRoleType, values_callable=lambda x: [e.value for e in x]), default=UserRoleType.EMPLOYEE)
    status = Column(SQLEnum(UserStatusType, values_callable=lambda x: [e.value for e in x]), default=UserStatusType.ACTIVE)
...
class UserToolAccess(Base):
    ...
    status = Column(SQLEnum(AccessStatusType, values_callable=lambda x: [e.value for e in x]), default=AccessStatusType.ACTIVE)
...
class AccessRequest(Base):
    ...
    status = Column(SQLEnum(RequestStatusType, values_callable=lambda x: [e.value for e in x]), default=RequestStatusType.PENDING)
